...
function ControlledExample() {
  ...
  return (
    <Box css={{ maxWidth: '25ch' }}>
      <DateRangePicker aria-label="delivery date" onChange={setValue} value={value} />
    </Box>
  );
}
```
...
function UnavailableExample() {
  ...
  return (
    <Box css={{ maxWidth: '25ch' }}>
      <DateRangePicker aria-label="delivery date" isDateUnavailable={isDateUnavailable} />
    </Box>
  );
}
```
...
```js
import { DateRangePicker } from '@project44-manifest/react';
```
